...
Ghola Loop Code Version - 1.5 - Jan 2025 - Functions, no new elements
...
void loop() {                   // enable the motor
  ...
  if (train_loop == 1) {  // if train_loop value is 1 run this
    ...
    if (digitalRead(Sensor_before_station) == LOW) {  // check if Sensor_at_station was triggered and then run this code
...
void train_move(int train_speed) {  // train_move function
  ...
  lcd.setCursor(13, 0);   // last character on the first LCD line
  lcd.print("ST");        // print x on the LCD screen
  lcd.print(at_station);  // show at_station value

  if ((at_station == 0 && train_speed == 100) || (at_station == 1 && train_speed == 0)) {  // if at_station is 0 and train_speed is 100 OR at_station is 1 and train_speed is 0 run this code
